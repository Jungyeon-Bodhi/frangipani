...
class Preprocessing:
    ...
    def grouping(self):
        df = self.df
        df['5'] = df[['5-1', '5-2', '5-3', '5-4']].bfill(axis=1).iloc[:, 0]
        df.loc[(df["E1"] == "Irène") & (df["today"] == "2025-03-21"), "5"] = "Makamba"
        df.loc[(df["E1"] == "Kamanda Alexis") & (df["today"] == "2025-03-21"), "5"] = "Makamba"
        self.df = df
        
    def enuemrator_unify(self):
        df = self.df
        df["E1. Enumerator Name"] = df["E1. Enumerator Name"].str.title()
        mapping = {
            "Manariyo.Freddy":"Manariyo Freddy",
            "Kabageni Mireille ":"Kabageni Mireille",
            "Rikundo Sonia":"Rukundo Sonia",
            "Kamanda Alexis ":"Kamanda Alexis",
            "Nizigiyimana Josélyne ":"Nizigiyimana Josélyne",
            "Irene":"Irène",
            "Irène ":"Irène",
            "Kmanda Alexis":"Kamanda Alexis",
            "Nizigiyimana Joselyne":"Nizigiyimana Josélyne",
            "Furaha Jean Paul ":"Furaha Jean Paul",
            "Ndaizamba Émilie":"Ndayizamba Emile",
            "Haziyo":"Haziyo Gamy Ghyslain",
            "Emile Ndayizamba":"Ndayizamba Emile",
            "Ndayizamba":"Ndayizamba Emile",
            "Irakoze Belyse": "Belyse Irakoze",
            }
        df["E1. Enumerator Name"] = df["E1. Enumerator Name"].replace(mapping)
        df.loc[
    df["4. Province of residence"] == "Rumonge","1. Category"] = "Women in Action participant"
        self.df = df 
        
    def version_update(self):
        df = self.df
        old_version = "vFbKwHAhAuUQvwE8gfWujr"
        ...
        enumerators = ["Lily Kaze"]
        
        columns = ["E3. Did the respondent consent to take part in the interview?",
                   "6. Have you ever been to school ?",
                   "7. Are you still at school?",
                   "18. Are you a member of an economic association/group (VSLA, economic interest group, AVEC, SILCS or savings and credit group)?",
                   "19. Are you a member of a civil society organisation (either local or national)?",
                   "21. If yes, did you participate in initiatives to support the inclusion of women in the economy and to support gender equality?",
                   "38. Do you know how to report any harm or concerns that you or someone else may have experienced while participating in the activity/project?"]
        
        mask = (df["E1. Enumerator Name"].isin(enumerators)) & (df["__version__"] == old_version)
        
        df.loc[mask, "3. Sex"] = df.loc[mask, "3. Sex"].map(gender)
        
        for col in columns:
            df.loc[mask, col] = df.loc[mask, col].map(yesno)
        
        print(f"{mask.sum()} data points have been updated")
        self.df = df  
        
    def indicator_cal(self):
        df = self.df
        df["Ind1.3"] = df['25'].map(lambda x: "Adequate" if x in ["Yes, sometimes"] else ("Inadequate" if x in ["Never", "Rarely"] else pd.NA))
